...
namespace HackerRank
{
    ...
    class MRotate : TProblem<MRotateSample, MRotateAnswer>
    {
        ...
        static void RotateRing(List<List<Int32>> arr, Int32 Ring, Int32 Rotates)
        {
            ...
            Int32 OldX = Ring;
            Int32 OldY = Ring;
            Int32 Val = arr[OldX][OldY];


            if (Rotates * 2 == RingLength)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < RingLength; i++)
                {
                    Int32 NewX = Ring;
                    Int32 NewY = Ring;
                    Int32 NewShift = (OldShift + Rotates) % RingLength;
                    if (NewShift > (2 * (MaxX - Ring) + (MaxY - Ring)))
                    {
                        NewX = Ring;
                        NewY = MaxY - (NewShift - (2 * (MaxX - Ring) + (MaxY - Ring)));
                    }
                    else
                    {
                        if (NewShift > (MaxX + MaxY - 2 * Ring))
                        {
                            NewY = MaxY;
                            NewX = MaxX - (NewShift - (MaxX - Ring) - (MaxY - Ring));
                        }
                        else
                        {
                            if (NewShift > MaxX - Ring)
                            {
                                NewX = MaxX;
                                NewY = Ring + NewShift - (MaxX - Ring);
                            }
                            else
                            {
                                NewY = Ring;
        ...
        void PrintMatrix(List<List<Int64>> arr)
        {
        ...
        static void RotateRing2(List<List<Int64>> arr, Int32 Ring, Int64 Rotates) {
            ...
            for (int i = 1; i < RingLength; i++)
            {
                ...
                {
                    NewX = Ring;
                    NewY = MaxY - (i - (2 * (MaxX - Ring) + (MaxY - Ring)));
                }
                else
                {
                    if (i > (MaxX + MaxY - 2 * Ring))
                    {
                        NewY = MaxY;
                        NewX = MaxX - (i - (MaxX - Ring) - (MaxY - Ring));
                    }
                    else
                    {
                        if (i > MaxX - Ring)
                ...
                Next = LRing.AddAfter(Next, arr[NewX][NewY]);
                if (i % Rotates == 0)
                {
                    newFirst = Next;
                }
        ...
        static void RotateRing3(List<List<Int64>> arr, Int32 Ring, Int32 Rotates)
        {
            Int32 RingLength = 2 * (arr.Count - 2 * Ring - 1) + 2 * (arr[0].Count - 2 * Ring - 1);
            Rotates = Rotates % RingLength;
            if (Rotates == 0)
            ...
            Int64[] ARing = new Int64[RingLength];
            Int32 ARingIndex = Rotates;
            ...
            for (int i = 0; i < RingLength; i++)
            {
                ...
                ARing[ARingIndex] = arr[NewX][NewY];
                ARingIndex++;
                if (ARingIndex == RingLength)
                {
                    ARingIndex = 0;
                }
            }
